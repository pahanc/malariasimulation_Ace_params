...
get_parameters <- function(overrides = list()) {
  ...
  mosq_suppression<-unlist(read.csv("input_files/mosq_suppression.csv",header=F,colClasses="numeric"))
  dimnames(mosq_suppression)<-NULL
  mosq_suppression<-as.vector(mosq_suppression)
  
  mosq_seasonality<-unlist(read.csv("input_files/mosq_seasonality2.csv",header=F,colClasses="numeric"))
